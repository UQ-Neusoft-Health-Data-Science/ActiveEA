...
from openea.modules.finding.alignment import greedy_alignment, greedy_alignment_exp
...
def test_exp(embeds1, embeds2, mapping, top_k, threads_num, metric='inner', normalize=False, csls_k=0, accurate=True):
    if mapping is None:
        alignment_rest_12, hits_12, mr_12, mrr_12, ndcgs = greedy_alignment_exp(embeds1, embeds2, top_k, threads_num,
                                                                      metric, normalize, csls_k, accurate)
    else:
        test_embeds1_mapped = np.matmul(embeds1, mapping)
        alignment_rest_12, hits_12, mr_12, mrr_12, ndcgs = greedy_alignment_exp(test_embeds1_mapped, embeds2, top_k, threads_num,
                                                                      metric, normalize, csls_k, accurate)
    return alignment_rest_12, hits_12, mr_12, mrr_12, ndcgs
