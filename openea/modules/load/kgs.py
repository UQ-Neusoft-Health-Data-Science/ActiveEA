...
class KGs:
    def __init__(self, kg1: KG, kg2: KG, train_links, test_links, valid_links=None, mode='mapping', ordered=True, ent1_labels=None):
        if mode == "sharing":
        ...
        if ent1_labels:
            self.train_ent1_id_labels = np.array([(ent_ids1[ent1], lbl) for ent1, lbl in ent1_labels["train_ent_label"]])
            self.valid_ent1_id_labels = np.array([(ent_ids1[ent1], lbl) for ent1, lbl in ent1_labels["valid_ent_label"]])
            self.test_ent1_id_labels = np.array([(ent_ids1[ent1], lbl) for ent1, lbl in ent1_labels["test_ent_label"]])
...
def read_kgs_from_folder(training_data_folder, division, mode, ordered, remove_unlinked=False):
    ...
    ent_lbl_fn = os.path.join(training_data_folder, division, "ent_labels.json")
    if os.path.exists(ent_lbl_fn):
        with open(ent_lbl_fn) as file:
            ent1_lbl_obj = json.loads(file.read())
    else:
        ent1_lbl_obj = None
    kgs = KGs(kg1, kg2, train_links, test_links, valid_links=valid_links, mode=mode, ordered=ordered, ent1_labels=ent1_lbl_obj)
    return kgs
