...
class EntityClassifier(BasicModel):
    ...
    def set_args(self, args):
        ...
        self.out_folder = self.args.output
    ...
    def _get_variable(self):
        self.init_embedding = tf.get_variable('init_embedding',
                                              ...
                                              initializer=tf.glorot_uniform_initializer(),
                                              dtype=tf.float32)
        self.ent_cls_w = tf.get_variable("ent_cls_w", shape=(), initializer=tf.glorot_uniform_initializer(), dtype=tf.float32)
        self.ent_cls_b = tf.get_variable("ent_cls_b", shape=(), initializer=tf.glorot_uniform_initializer(), dtype=tf.float32)
    ...
    def compute_loss(self, pos_links, neg_links, ent1, lbls, only_pos=False):
        index1 = pos_links[:, 0]
        index2 = pos_links[:, 1]
        ...
        embeds1 = tf.nn.embedding_lookup(output_embeds, tf.cast(ent1, tf.int32))
        embeds2 = tf.nn.embedding_lookup(output_embeds, tf.cast(self.kgs.useful_entities_list2, tf.int32))
        simi_mtx = tf.linalg.matmul(embeds1, embeds2, transpose_b=True)  # ent1_num x ent2_num
        max_simi = tf.math.reduce_max(simi_mtx, axis=1)
        logits = tf.math.add(tf.math.multiply(self.ent_cls_w, max_simi), self.ent_cls_b)
        probs = tf.keras.activations.sigmoid(logits)
        ent_cls_loss = tf.keras.losses.binary_crossentropy(y_true=lbls, y_pred=probs, from_logits=False)
        self.probs = probs
    ...
    def _generate_rel_graph(self):
        ...
        self.ent1 = tf.placeholder(tf.int32, shape=[None], name="ent1")
        self.lbl = tf.placeholder(tf.int32, shape=[None], name='lbl')
        self._define_model()
        # self.loss = self.compute_loss(self.rel_pos_links, self.rel_neg_links) + \
        #             self.compute_rel_loss(self.hs, self.ts)
        self.loss = self.compute_loss(self.rel_pos_links, self.rel_neg_links, self.ent1, self.lbl)
        self.optimizer = tf.train.AdamOptimizer(learning_rate=self.args.learning_rate).minimize(self.loss)
    ...
    def save(self):
        ...
        # save model
        model_fn = os.path.join(self.out_folder, "model.ckpt")
        saver = tf.compat.v1.train.Saver()
        saver.save(self.session, model_fn)
        ...
        if self.args.dropout == 0:
            ...
            for i in range(10):
                ...
                for output_embeds in [input_embeds] + output_embeds_list:
                    ...
                    embeds_list.append(output_embeds)
                ent_embeds = np.concatenate(embeds_list, axis=1)
                sub_dir = os.path.join(self.out_folder, "model_drop_%02d" % i)
                if not os.path.exists(sub_dir):
                    os.mkdir(sub_dir)
                if not sub_dir.endswith("/"):
                    sub_dir += "/"
                rd.save_embeddings(sub_dir, self.kgs, ent_embeds, None, None, mapping_mat=None)
    ...
    def generate_ent1_batch(self, ent1_batch_size):
        ent1_idxes = np.random.choice(len(self.kgs.train_ent1_id_labels), ent1_batch_size)
        ent1_lbl_batch = self.kgs.train_ent1_id_labels[ent1_idxes]
        return ent1_lbl_batch[:, 0], ent1_lbl_batch[:, 1]
    ...
    def generate_neg_instances_for_bachelors(self):
        print("#### ====> generate negative links for bachelors")
        bach_fn = os.path.join(self.args.training_data, self.args.dataset_division, "bachelors.json")
        with open(bach_fn) as file:
            bach_obj = json.loads(file.read())
        bachelor_ent_list = [self.kgs.kg1.entities_id_dict[uri] for uri in bach_obj]
        output_embeds_list = self.output_embeds_list
        ents2 = self.sup_ent2 + self.ref_ent2
        embeds1 = tf.nn.embedding_lookup(output_embeds_list[-1], bachelor_ent_list)
        embeds2 = tf.nn.embedding_lookup(output_embeds_list[-1], ents2)
        embeds1 = tf.nn.l2_normalize(embeds1, 1)
        embeds2 = tf.nn.l2_normalize(embeds2, 1)
        embeds1 = np.array(embeds1.eval(session=self.session))
        ...
        idx_argsort = np.argsort(-sim_mat)
        idx_argsort = idx_argsort[:, :self.args.neg_triple_num]
        bachelor_neg_links = []
        for i in range(len(bachelor_ent_list)):
            tmp_ent2_list = [ents2[ent2_idx] for ent2_idx in idx_argsort[i]]
            bachelor_neg_links.extend(list(zip([bachelor_ent_list[i]]*self.args.neg_triple_num, tmp_ent2_list)))
        return np.array(bachelor_neg_links)

    def cls_ent(self, ent1):
        fetches = {"ent_lbl_prob": self.probs}
        feed_dict = {
            self.ent1: ent1
        }
    ...
    def run(self):
        ...
        print("####### Observe data num: ", len(self.sup_ent2))
        ...
        ent1_batch_size = int(np.ceil(len(self.kgs.train_ent1_id_labels) / steps))

        for epoch in range(1, self.args.max_epoch + 1):
            start = time.time()
            epoch_loss = 0.0
            # if os.path.exists(bach_fn):  # add bachelor neg links
            #     bachelor_neg_links = self.generate_neg_instances_for_bachelors()
            #     bach_batch_size = int(np.ceil(len(bachelor_neg_links) / steps))
            for step in range(steps):
                ...
                self.ent1_batch, self.lbl_batch = self.generate_ent1_batch(ent1_batch_size)

                # if os.path.exists(bach_fn):  # add bachelor neg links
                #     idxes = np.random.choice(len(bachelor_neg_links), bach_batch_size)
                #     bach_neg_links_batch = bachelor_neg_links[idxes]
                #     self.neg_link_batch = np.concatenate([self.neg_link_batch, bach_neg_links_batch], axis=0)
                ...
                if self.args.rel_param > 0:
                    ...
                    feed_dict = {self.rel_pos_links: self.pos_link_batch,
                                 ...
                                 self.hs: hs,
                                 self.ts: ts,
                                 self.ent1: self.ent1_batch,
                                 self.lbl: self.lbl_batch
                                 }
                    results = self.session.run(fetches=fetches, feed_dict=feed_dict)
                else:
        ...
        # log num of training data
        steps_log_fn = os.path.join(self.out_folder, "training_steps_log.txt")
        if not os.path.exists(steps_log_fn):
            with open(steps_log_fn, "w+") as file:
                file.write(str(len(self.sup_ent2)))
        else:
            with open(steps_log_fn, "r") as file:
                cur_step_num = int(file.read().strip())
            with open(steps_log_fn, "w+") as file:
                file.write(str(cur_step_num+len(self.sup_ent2)))
