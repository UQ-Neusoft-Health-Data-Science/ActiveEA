...
import os
from openea.modules.finding.evaluation import test_exp
import json
...
class RDGCN(BasicModel):
    ...
    def init(self):
        self.word_embed = os.path.join(self.args.training_data, "../../wiki-news-300d-1M.vec")  # before ._get_desc_input()
    ...
    @classmethod
    def cache_word_embs(cls, training_data_dir):
        word_embed_fn = os.path.join(training_data_dir, "../../wiki-news-300d-1M.vec")
        with open(word_embed_fn, 'r') as f:
            w = f.readlines()
            w = w[1:]
        word = []
        emb_list = []
        for line in tqdm(w, "parsing lines"):
            tokens = line.split(" ")
            word.append(tokens[0])
            emb_list.append(tokens[1:-1])
        word_em = np.array(emb_list, dtype=np.float)
        cls.ext_vocab = word
        cls.ext_word_embs = word_em
    ...
    def _get_desc_input(self):
        # desc graph settings
        print("## ==> start getting desc input")
        start = time.time()
        model = self
        print("start getting local name by name triple")
        name_triples = self._get_local_name_by_name_triple()
        print("end getting local name by name triple")
        ...
        def load_word_embs():
            ...
            word_em = np.array(emb_list, dtype=np.float)
            return word, word_em
        if RDGCN.ext_vocab is None:
            print("###====== RDGCN does not have word embs. loading now  ======###")
            RDGCN.ext_vocab, RDGCN.ext_word_embs = load_word_embs()
        else:
            print("###====== use cached word embs by RDGCN ======###")
        word, word_em = RDGCN.ext_vocab, RDGCN.ext_word_embs
        ...
        all_w_list = word + un_logged_words.tolist()
        all_id_list = list(range(len(word))) + [un_logged_id, ] * len(un_logged_words)
        all_word = pd.Series(data=all_id_list, index=all_w_list)
    ...
    def test(self, save=True):
        ...
        rest_12, hits_12, mr_12, mrr_12, ndcgs = test_exp(embeds1, embeds2, None, self.args.top_k,
        ...
        # _, hits_12, mr_12, mrr_12, ndcgs = test_exp(embeds1, embeds2, None, self.args.top_k,
        #                                             self.args.test_threads_num,
        #                                             metric=self.args.eval_metric, normalize=self.args.eval_norm,
        ...
        if save:
            ent_ids_rest_12 = [(self.kgs.test_entities1[i], self.kgs.test_entities2[j]) for i, j in rest_12]
            rd.save_results(self.out_folder, ent_ids_rest_12)
            with open(os.path.join(self.out_folder, "metrics.json"), "w+") as file:
        ...
        if self.args.dropout == 0:
            ...
            for i in range(10):
                ...
                if not sub_dir.endswith("/"):
                    sub_dir += "/"
                rd.save_embeddings(sub_dir, self.kgs, embedding, None, None, mapping_mat=None)


    def restore(self):
        model_fn = os.path.join(self.out_folder, "model.ckpt")
        if not os.path.exists(model_fn+".meta"):
            print("no model file to restore from")
            return
        saver = tf.compat.v1.train.Saver()
        saver.restore(self.sess, model_fn)
        print("loaded model successfully from "+model_fn)
    ...
    def valid(self, stop_metric):
        return self.valid_(stop_metric)
