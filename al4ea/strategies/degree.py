...
class DegreeStrategy(Strategy):
    def __init__(self, data_dir, al_settings: ALSettings):
        super(DegreeStrategy, self).__init__()

        graph = construct_graph(data_dir, al_settings.edge_mode)
        self.ent2centrality_map = degree_centrality(G=graph)
...
def al_process_with_struct_uncertainty():
    ...
    strategy = DegreeStrategy(data_dir=args.data_dir, al_settings=al_settings)
