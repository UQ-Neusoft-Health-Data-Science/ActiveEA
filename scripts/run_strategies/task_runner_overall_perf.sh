...
do
  for model_name in "Alinet" "BootEA" "RDGCN"
  do
    for bachelor_percent in 0.0 0.3
    do
      tmp_setting_fn=$tmp_dir/${seed}_${task_group}_${data_name}_bach${bachelor_percent}_eamodel${model_name}_alpha${alpha}.sh
      ...
      echo "task_group=overall_perf" >> $tmp_setting_fn
      echo "data_size=15K" >> $tmp_setting_fn
      echo "seed=${seed}" >> $tmp_setting_fn
...
do
  ...
  do
    ...
    do
      ...
      echo "task_group=overall_perf" >> $tmp_setting_fn
      echo "data_size=15K" >> $tmp_setting_fn
      echo "sample_num_per_ite=100" >> $tmp_setting_fn
      echo "seed=${seed}" >> $tmp_setting_fn
